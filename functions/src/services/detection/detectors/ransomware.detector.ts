...
import {DetectionResult, ProcessInfo, FileChangeInfo} from "../detection.types";
import {Server} from "../../../models/server.model";
// import * as admin from "firebase-admin"; // No usado actualmente

export class RansomwareDetector extends BaseDetectorImpl {
   ...
   */
  private detectSuspiciousProcesses(
    serverId: string,
  ...
  private detectMassEncryption(
    ...
    const suspiciousExtensions = recentChanges.filter((change) => {
      return this.suspiciousExtensions.some((ext) =>
        change.path.toLowerCase().endsWith(ext),
      );
    });
   ...
   */
  private detectHighCpuWithFileChanges(
    serverId: string,
    metrics: any,
  ): DetectionResult | null {
    const highCpu = metrics.cpuUsage > 80;
    const manyFileChanges = metrics.fileChanges?.length > 20;
