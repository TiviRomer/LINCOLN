...
import {BaseDetectorImpl} from "./base.detector";
import {DetectionResult, NetworkConnectionInfo} from "../detection.types";
import {Server} from "../../../models/server.model";

export class IntrusionDetector extends BaseDetectorImpl {
  private readonly suspiciousPorts = [22, 23, 3389, 5900, 1433, 3306, 5432]; // SSH, Telnet, RDP, VNC, SQL
  private readonly maxFailedLogins = 5; // Intentos fallidos antes de alertar

  async detect(serverId: string, _server: Server): Promise<DetectionResult[]> {
    ...
    console.log(`   networkConnections: ${metrics.networkConnections?.length || 0}`);
    ...
    const suspiciousConnectionResult = this.detectSuspiciousConnections(
      serverId,
    ...
    if (suspiciousConnectionResult) {
      ...
      console.log(`   ℹ️  IntrusionDetector: No se detectaron conexiones sospechosas (${metrics.networkConnections?.length || 0} conexiones)`);
    }
   ...
   */
  private detectFailedLoginAttempts(
    serverId: string,
    metrics: any,
  ...
  private detectPortScanning(
    ...
    const connectionsByIP = new Map<string, NetworkConnectionInfo[]>();
    ...
    for (const [ip, conns] of connectionsByIP.entries()) {
      if (this.isPrivateIP(ip)) continue; // Ignorar IPs privadas (pueden ser legítimas)
