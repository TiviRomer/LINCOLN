...
import {DetectionResult, ProcessInfo} from "../detection.types";
import {Server} from "../../../models/server.model";

export class AnomalyDetector extends BaseDetectorImpl {
  private readonly cpuThreshold = 90; // Porcentaje
  private readonly memoryThreshold = 90; // Porcentaje
   ...
   */
  private async detectAnomalousCPU(
    serverId: string,
    metrics: any,
   ...
   */
  private async detectAnomalousMemory(
    serverId: string,
    metrics: any,
  ): Promise<DetectionResult | null> {
    // Primero verificar umbral absoluto
   ...
   */
  private detectAnomalousProcesses(
    serverId: string,
    processes: ProcessInfo[],
  ): DetectionResult | null {
    // Procesos consumiendo recursos excesivos
    const highResourceProcesses = processes.filter((proc) =>
      proc.cpuUsage > 50 || proc.memoryUsage > 20,
    );
   ...
   */
  private detectOffHoursActivity(
    serverId: string,
    metrics: any,
  ): DetectionResult | null {
    const now = new Date();
   ...
   */
  private async detectUsagePatternChanges(
    serverId: string,
    metrics: any,
  ): Promise<DetectionResult | null> {
    ...
    if (currentHourAvg && currentHourAvg.length > 0) {
      ...
      const variance = currentHourAvg.reduce(
        (sum, val) => sum + Math.pow(val - avgForHour, 2),
        0,
      ) / currentHourAvg.length;
