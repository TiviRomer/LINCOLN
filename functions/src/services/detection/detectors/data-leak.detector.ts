...
export class DataLeakDetector extends BaseDetectorImpl {
  private readonly dataLeakThreshold = 100 * 1024 * 1024; // 100 MB
  ...
  async detect(serverId: string, _server: Server): Promise<DetectionResult[]> {
    ...
    const cloudStorageResult = this.detectCloudStorageConnections(
      serverId,
   ...
   */
  private async detectMassiveDataTransfer(
    serverId: string,
    metrics: any,
    ...
    if (historicalMetrics.length === 0 || historicalMetrics.length === 1) {
      ...
      return {
        ...
        severity: this.calculateSeverity(
          confidence,
          metrics.networkOut > this.dataLeakThreshold * 5,
        ),
   ...
   */
  private detectCloudStorageConnections(
    serverId: string,
    ...
    const httpsConnections = connections.filter((conn) =>
      conn.protocol === "tcp" &&
   ...
   */
  private async detectUnusualNetworkActivity(
    serverId: string,
    metrics: any,
  ): Promise<DetectionResult | null> {
    ...
    const variance = networkOutValues.reduce(
      (sum, val) => sum + Math.pow(val - avg, 2),
      0,
    ) / networkOutValues.length;
